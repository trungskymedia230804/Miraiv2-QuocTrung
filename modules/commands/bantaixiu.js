...
// --- thay block cũ ---
/*
let statusPath = __dirname + '/cache/data/status-hack.json';
let data = {};
...
// --- bằng block mới ---
const statusPath = path.join(__dirname, "cache", "data", "status-hack.json");
let data = {};
const save = () => fs.writeFileSync(statusPath, JSON.stringify(data));

if (fs.existsSync(statusPath)) {
  data = JSON.parse(fs.readFileSync(statusPath));
} else {
...
const dice_photos = [
  path.join(__dirname, "assets", "Q3QfE4t.jpeg"),
  path.join(__dirname, "assets", "M3juJEW.jpeg"),
  path.join(__dirname, "assets", "Tn6tZeG.jpeg"),
  path.join(__dirname, "assets", "ZhOA9Ie.jpeg"),
  path.join(__dirname, "assets", "eQMdRmd.jpeg"),
  path.join(__dirname, "assets", "2GHAR0f.jpeg")
];
...
exports.handleEvent = async o => {
  ...
  if (select == 'o') {
      ...
      //await send({ body: `Xúc xắc: ${dices.join('|')} - ${sum} điểm (${select_values[winner]})\nNhững người thắng:\n${winner_players.map(($, i) => (crease_money = $.bet_money * BigInt(String(rate)), o.Currencies.increaseMoney($.id, crease_money.toString()), `${i + 1}. ${global.data.userName.get($.id)} chọn (${select_values[$.select]})\n+${crease_money.toLocaleString()}$`)).join('\n')}\n\nNhững người thua:\n${lose_players.map(($, i) => (o.Currencies.decreaseMoney($.id, $.bet_money.toString()), `${i + 1}. ${global.data.userName.get($.id)} chọn (${select_values[$.select]})\n-${$.bet_money.toLocaleString()}$`)).join('\n')}\n\nChủ bàn: ${global.data.userName.get(d[tid].author)}`, attachment: dices.map($ => dice_stream_photo[$]), });
        ...
        const attachments = dices.map(num => {
  const fp = dice_photos[num - 1]; // num là 1..6
  return fs.existsSync(fp) ? fs.createReadStream(fp) : null;
}).filter(Boolean);

await send({
  body: `Xúc xắc: ${dices.join('|')} - ${sum} điểm (${select_values[winner]})\n` +
        `Những người thắng:\n` +
        `${winner_players.map(($, i) => (crease_money = $.bet_money * BigInt(String(rate)),
          o.Currencies.increaseMoney($.id, crease_money.toString()),
          `${i + 1}. ${global.data.userName.get($.id)} chọn (${select_values[$.select]})\n+${crease_money.toLocaleString()}$`
        )).join('\n')}\n\n` +
        `Những người thua:\n` +
        `${lose_players.map(($, i) => (
          o.Currencies.decreaseMoney($.id, $.bet_money.toString()),
