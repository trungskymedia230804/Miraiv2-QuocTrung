...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "Trung Media",
  description: "Gửi ảnh trai random (trừ 5$ nếu thành công)",
  commandCategory: "Fun",
  usages: "trai",
  cooldowns: 3
};

// bộ link từ JSON bạn gửi (78 ảnh)
const BOY_LINKS = [
  ...
  "https://i.postimg.cc/VLS3mMTL/0223-52609139-2210277149192385-2811100977640243200-n.jpg",
  "https://i.postimg.cc/W1gCJVTB/0225-52688560-2210277012525732-5248890463133368320-n.jpg",
...
module.exports.run = async function ({ api, event, Currencies }) {
  ...
  if (bal < COST)
    return api.sendMessage(`⚠️ Bạn không đủ tiền (${COST}$) để xem ảnh trai đâu 😢`, threadID, messageID);

  // chọn link ngẫu nhiên
  const url = BOY_LINKS[Math.floor(Math.random() * BOY_LINKS.length)];

  try {
    // tải ảnh; nếu ok mới trừ tiền
    ...
    return api.sendMessage(
      {
        body: `Trai đẹp nè 😎\n- Đã trừ ${COST}$ (còn: ${formatMoney(left)}$)`,
        attachment: stream
      },
      threadID,
      messageID
    ...
    console.error(e);
    return api.sendMessage("❌ Lỗi tải ảnh hoặc link hỏng. Thử lại nhé!", threadID, messageID);
  }
};
