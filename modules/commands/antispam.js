...
module.exports.languages = {
  vi: {
    ...
    confirm: "Thả '❤️' để xác nhận đổi",
    done: "Hoàn tất",
    error: "Đã xảy ra lỗi",
    spamKick: "Spam nên kick %1"
  },
  en: {
    on: "On",
    off: "Off",
    noData: "No data",
    confirm: "React '❤️' to confirm",
    done: "Done",
    error: "An error occurred",
    spamKick: "Kicked due to spam: %1"
  }
};
...
module.exports.handleEvent = async function({ api, event, args, Users, getText }) {
			 ...
			 if(findThread.status){
					if (!userSpamBlocker.canSend(userId,findThread)) {
						const name = (await Users.getData(userId)).name
						 api.sendMessage(getText('spamKick', name), event.threadID);
