module.exports.config = {
  ...
  version: '2.2.0',
  hasPermssion: 0,
  credits: 'DC-Nam mod by gojo & fix by Khôi',
...
module.exports.run = async function({ api, event, args, permssion }) {
  const { sendMessage: send, unsendMessage: un, shareContact: share } = api;
  ...
  if (args.length >= 1) {
    if (args[0].toLowerCase() === 'all') {
      return sendFullCommandList(send, un, tid, mid, isAdmin, isGroupAdmin, permssion);
    }
    ...
    txt += `╰━━━━━━━━━━━━━━━━╯\n` +
           ...
           `╰━━━━━━━╯\n` +
           `📝 Reply từ 1 đến ${data.length} để xem chi tiết\n` +
           `💡 Gõ "menu all" để xem tất cả lệnh\n` +
...
function sendFullCommandList(send, un, tid, mid, isAdmin, isGroupAdmin, permssion) {
  ...
  cmds.forEach((cmd, index) => {
    ...
    const name = cmd?.config?.name || `lenh${index + 1}`;
    txt += `┃ ${index + 1}. ${icon} ${name}\n`;
  });
