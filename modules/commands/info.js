...
const tokens = JSON.parse(fs.readFileSync(`${process.cwd()}/bot/data/tokens.json`, 'utf-8'));
...
async function getProfileCoverPhoto(uid) {
  ...
  try {
    ...
    return matches && matches.length > 1 ? matches[1] : "không có";
  } catch (e) {
    return "Không có";
  }
}
...
│ Giới tính: ${resp.data.gender == "male"
              ? "Nam"
...
this.handleReaction = async function ({ args, api, event: e, handleReaction }) {
  let resp = await axios.get(
    `https://graph.facebook.com/${handleReaction.author}?fields=id,likes,family,posts&access_token=${accessToken}`
  );
