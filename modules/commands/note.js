...
module.exports = {
  config: {
    ...
    description: 'https://niio-team.onrender.com/note/:UUID',
  ...
  run: async function (o) {
    ...
    try {
      if (/^https:\/\//.test(url)) {
        ...
        const url_raw = new URL(`https://buda-juoe.onrender.com/note/${uuid_raw}`);
        const url_redirect = new URL(`https://buda-juoe.onrender.com/note/${require('uuid').v4()}`);
